...
class YTDL {
  ...
  async downloadAudio(url) {
    const info = await this.getVideoInfo(url, 'MP3')
    ...
    try {
      ...
      if (!text) {
        ...
        await ryza.sendMessage(from, { 
          text: `Contoh: .ytmp3 https://youtu.be/xxxxx` 
        }, { quoted: msg })
        return
      }
      ...
      const result = await scraper.downloadAudio(ytUrl)
      
      await react(ryza, from, msg, "✅")
      
      await ryza.sendMessage(from, {
        audio: { url: result.secureUrl },
        mimetype: "audio/mpeg",
